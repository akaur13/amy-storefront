...
gulp.task('babelscripts', ()=> 
   gulp.src('.tmp/scripts/scripts.js', {base: './'})
  .pipe(babel())
  .pipe(gulp.dest('./'))
);
...
gulp.task('test', function() {
  runSequence('styles', 'extractAndAnnotate', 'html2js', 'babelscripts', 'test-core', 'watch-test');
});
...
gulp.task('test-ut', function() {
  //exclude hack
  var testToFiles = paths.testRequire.concat(paths.templates, ['!' + paths.hackScript]);
...
gulp.task('jstask', function() {
  return runSequence('lint', 'lint-test-fail','extractAndAnnotate', 'html2js', 'babelscripts', 'test-core', 'appjsmin', 'vendorjsmin', 'reversion');
});
...
gulp.task('_build', ['clean:dist'], function() {
  runSequence(['images', 'copy:extras', 'copy:fonts', 'client:build'], 'build-meta');
});
...
gulp.task('create-artifact', () =>{
   return gulp.src('dist/**/*')
        .pipe(archiver('frontend-v' + pckg.version + 'd' + new Date().getTime() + '.zip'))
        .pipe(gulp.dest('./dist'));
});

gulp.task('build-meta', function(cb){
  fs.writeFile('./dist/meta.txt',  ['/**',
  ' * @pkg.name- '+ pckg.name ,
  ' * @version- v' + pckg.version ,
  ' * @artifact- frontend-v' + pckg.version + '.zip',
  ' * @buildTime- ' + new Date(),
  ' */',
  ''].join('\n'), cb);
});
